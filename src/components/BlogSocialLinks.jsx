...
const anchorLogoSrc = url.resolve(process.env.PUBLIC_URL, "/blog/anchorfm-logo.png");
...
const BlogSocialLinks = ({type, links}) => {
      ...
      <div className="social-links-container">
        <div className="social-links-grid">
          {linksToRender.map(({type, link}) => (
            <a href={link} target="_blank" rel="noopener noreferrer">{LINK_ICON_MAP[type]}</a>
          ))}
        </div>
      </div>
    );
  ...
  return (
    <div className="social-links-container">
      <a href={link} target="_blank" rel="noopener noreferrer">{LINK_ICON_MAP[linkType]}</a>
    </div>
  );
};
